...
>>>>>>> 51fa0337771e8e1ec249745c7bbb0e4b1d9e20ce
  ...
  return (
    <div className="mt-6 bg-white rounded-xl shadow-sm border border-black p-8 max-w-4xl">
      <h3 className="text-lg font-bold text-black mb-8">Setting Harga Per Halaman</h3>
      <div className="grid grid-cols-2 gap-6 mb-8">
        <div>
          <label className="block text-sm font-bold text-black mb-2">Harga Halaman BW (IDR)</label>
          <div className="flex border border-black rounded-lg overflow-hidden px-4 bg-white h-12 items-center">
            <span className="font-medium text-gray-500 mr-2">Rp</span>
            <input type="number" value={priceBw} onChange={(e) => setPriceBw(Number(e.target.value))}
              className="flex-1 font-medium outline-none text-black bg-transparent w-full" />
          </div>
        </div>
        <div>
          <label className="block text-sm font-bold text-black mb-2">Harga Halaman Warna (IDR)</label>
          <div className="flex border border-black rounded-lg overflow-hidden px-4 bg-white h-12 items-center">
            <span className="font-medium text-gray-500 mr-2">Rp</span>
            <input type="number" value={priceColor} onChange={(e) => setPriceColor(Number(e.target.value))}
              className="flex-1 font-medium outline-none text-black bg-transparent w-full" />
          </div>
        </div>
      </div>
