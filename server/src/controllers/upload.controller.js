...
const uploadFile = asyncHandler(async (req, res) => {
  ...
  const session = await sessionService.markSessionUsed(sessionId, {
    filePath: file.path,
  ...
  if (io) {
    ...
    const target = session.socketId
      ? io.to(sessionId).to(session.socketId)
      : io.to(sessionId);

    target.emit(SOCKET_EVENTS.FILE_UPLOADED, payload);

    logger.info("File upload socket event emitted", {
      sessionId,
